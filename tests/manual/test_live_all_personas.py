"""
Live API validation — 20 queries per persona (6 personas, 120 total).
...
# brave_access
PERSONAS = {
    "nephilim_aegis":   True,    # brave only
    "nephilim_aurora":  True,    # brave only
    "nephilim_cipher":  True,    # brave only
    "nephilim_solace":  True,    # brave only
    "nephilim_nyx":     False,   # no MCP
...
# expected values match API source_type field: "brave_mcp" | "llm"
QUERIES = [
    # BRAVE — Weather (5)
    ("What is the weather in London today?",             "brave_mcp", "llm"),
    ("Will it rain tomorrow in Berlin?",                 "brave_mcp", "llm"),
    ("Current temperature in Tokyo",                    "brave_mcp", "llm"),
    ("What's the forecast for Paris this weekend?",     "brave_mcp", "llm"),
    ("Is it going to be cold tomorrow?",                "brave_mcp", "llm"),

    # BRAVE — News & Events (5)
    ("What are the latest news about AI?",              "brave_mcp", "llm"),
    ("Breaking news today",                             "brave_mcp", "llm"),
    ("What's new with SpaceX?",                         "brave_mcp", "llm"),
    ("Recent developments in renewable energy",         "brave_mcp", "llm"),
    ("What happened in the US elections recently?",     "brave_mcp", "llm"),

    # BRAVE — Stocks & Opinion (5)
    ("What is the current stock price of NVIDIA?",      "brave_mcp", "llm"),
    ("How is the S&P 500 doing today?",                 "brave_mcp", "llm"),
    ("What do analysts think about Tesla stock?",       "brave_mcp", "llm"),
    ("What are predictions for the housing market?",    "brave_mcp", "llm"),
    ("What is trending on Twitter right now?",          "brave_mcp", "llm"),
...
def get_expected(row, has_brave):
    _, exp_brave, exp_no_mcp = row
    return exp_brave if has_brave else exp_no_mcp
...
def run_persona(persona_key, has_brave):
    pass_count = 0
    fail_count = 0
    errors = []
    total_time = 0.0
    ...
    for i, row in enumerate(QUERIES, 1):
        query = row[0]
        expected = get_expected(row, has_brave)
        start = time.time()
        try:
...
def main():
    ...
    for persona_key, has_brave in PERSONAS.items():
        mcp_label = "brave" if has_brave else "none"

        print(f"\n{'-' * 110}")
        print(f"  {persona_key} | mcp=[{mcp_label}]")
        print(f"{'-' * 110}")

        p, f, errs, t = run_persona(persona_key, has_brave)
