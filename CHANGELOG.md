...
- **`SEARCH_SYNTHESIS_TRUST_RESULTS`** (default OFF): `build_synthesis_prompt` (`tools/synthesis_prompts.py`) adds a **RULE 0** (salience) + a reinforced closing line (recency, where small models attend most) stating the results were retrieved just now, are verified, and supersede earlier turns — "do not refuse/apologize based on anything said earlier in this conversation." Scoped with an explicit conditional so it never overrides RULE 5 honest abstention on genuinely empty/off-topic results (CRAG/Self-RAG pattern), and kept short/procedural + separate from voice-setting to avoid flattening persona. Off (default) = byte-identical synthesis prompt.
- **Live A/B verified** (prod flag-OFF vs worktree flag-ON, identical poisoned payload): OFF → "I cannot and will not perform web searches" (REFUSED); ON → "the search has been completed, seeker… switzerland has advanced to the round of 16" (ANSWERED), persona voice intact. Clean grounded answers unaffected (no regression). +4 headless tests (`test_synthesis_prompt.py`); full suite green. Prompt-hardening alone sufficed — history-stripping (the stronger lever) not needed.
