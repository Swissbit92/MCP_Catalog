...
**Single-model native tool brain (ADR-008 P1, flag-gated `TOOL_BRAIN_ENABLED`, default OFF).** The daily driver (abliterated Mistral-Small-24B) natively decides + fills tool calls via Ollama `/api/chat` `tools=`; `services/tool_brain_service.py` runs the loop — each call passes the reused ADR-004 `ToolCallInterceptor` (mcp gate + arg allowlist + hard-block `execute_swap` + HITL) before executing via ADR-009 registry-bound executors (`tools/executor_bindings.py`, with the per-persona safesearch clamp), then the SAME model synthesizes in-voice (no restyle handoff — single model). Native calling is phrasing-sensitive (TB0 spike: explicit phrasings call, colloquial ones miss ~40%), so `routes/chat.py:_try_tool_brain` returns None on a silent-but-tool-needed turn and falls through to the legacy deterministic force-search — that path is the reliability floor. Wallet stays on the existing propose→confirm→execute / `handle_wallet_query` flow. Supersedes the ADR-004 two-stage `AGENTIC` pipeline (which existed only because Magidonia couldn't native-tool-call); `argument_extractor.py` + the Stage1/Stage2 split are dead once this ships. See [ADR-008](docs/decisions/008-two-brain-split-tool-brain-voice-brain.md).
