# ui/ui_style.py
# Global CSS/JS injection + query param sync helpers (Cyberpunk CV banner)
...
def inject_global_css_js():
    ...
    .cv-banner{
      ...
      --panel-fg:    #e8f0ff;
      --grid-line:   rgba(150, 200, 255, 0.08);
      --grid-line-2: rgba(255, 120, 220, 0.06);
      ...
      background:
        /* neon radial glow corners */
        radial-gradient(260px 180px at 0% 0%, rgba(66,245,255,0.10), transparent 60%),
        radial-gradient(240px 160px at 100% 100%, rgba(255,100,246,0.10), transparent 60%),
        /* holographic diagonal */
      ...
      /* angled frame using clip-path (not a boring rectangle) */
      clip-path: polygon(
        12px 0%, calc(100% - 28px) 0%, 100% 18px, 100% calc(100% - 12px),
        calc(100% - 12px) 100%, 24px 100%, 0% calc(100% - 24px), 0% 12px
      );
      box-shadow:
        0 20px 40px rgba(0,0,0,0.45),
        0 0 0 1px rgba(255,255,255,0.04) inset,
        0 0 24px rgba(66,245,255,0.10),
        0 0 28px rgba(255,100,246,0.10);
    }
    ...
    /* moving light sweep already provided by .cv-sheen; strengthen cyber look */
    .cv-sheen{
      position: absolute; inset: 0;
      ...
      animation: cvSheen 5.2s infinite ease-in-out;
      pointer-events: none;
    ...
    /* bracket accents using drop-shadows on the clip */
    .cv-banner { filter: drop-shadow(0 0 6px rgba(66,245,255,0.25)) drop-shadow(0 0 10px rgba(255,100,246,0.15)); }
    ...
    .cv-avatar{
      ...
      box-shadow:
        ...
        0 0 22px rgba(66,245,255,0.18),
        0 0 26px rgba(255,100,246,0.14);
    }
    .cv-avatar-fallback{
      display:flex; align-items:center; justify-content:center;
      font-size: 48px; background: rgba(255,255,255,0.06);
    ...
    .cv-summary{
      ...
      text-wrap: pretty;
      text-shadow:
        0 0 2px rgba(66,245,255,0.25),
        0 0 8px rgba(154,107,255,0.12);
    }
    .cv-summary .cv-p{
      margin: 0 0 10px 0;
    }
    .cv-summary .cv-p:last-child{
      margin-bottom: 4px;
    }

    /* Dropcap on first paragraph with neon badge */
    .cv-summary .cv-lead::first-letter{
      ...
      background:
        linear-gradient(135deg, rgba(66,245,255,0.30), rgba(255,100,246,0.28));
      color: #0a0f18;
      box-shadow:
        inset 0 0 0 1px rgba(255,255,255,0.14),
        0 0 14px rgba(66,245,255,0.35),
        0 0 18px rgba(255,100,246,0.28);
    }

    /* Light theme fallback (if user forces light) */
    @media (prefers-color-scheme: light){
      .cv-banner{
        --panel-bg: rgba(240, 245, 255, 0.82);
        --panel-fg: #0f172a;
        color: var(--panel-fg);
        box-shadow:
          0 10px 24px rgba(0,0,0,0.12),
          0 0 0 1px rgba(0,0,0,0.04) inset,
          0 0 16px rgba(66,245,255,0.12),
          0 0 18px rgba(255,100,246,0.10);
      }
    ...
    @media (max-width: 1000px){
      .cv-banner{ grid-template-columns: 170px 1fr; }
    }
    @media (max-width: 740px){
      .cv-banner{ grid-template-columns: 140px 1fr; }
    }
