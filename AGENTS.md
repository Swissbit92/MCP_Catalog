...
## Tech Stack
- Backend: Python 3.8+, FastAPI, Pydantic, SQLite, Ollama
- Frontend: React 19, TypeScript 4.9.5, Tailwind CSS, Framer Motion
- Project Structure: `src/coordinator/` (API), `react-ui/` (React app), `ui/` (Streamlit UI)
...
- React/TS: `PascalCase` components, explicit TS types, strict mode. Hooks: `useThing`. Module-local utilities.
- Imports: Group stdlib → third-party → local. No wildcards. Use absolute imports in React.
...
- Layout: App uses `h-screen flex flex-col` with Header and `flex-1 overflow-hidden` content. Chat pages use `h-full`. Use CSS modules for component styles, Tailwind for utilities.
