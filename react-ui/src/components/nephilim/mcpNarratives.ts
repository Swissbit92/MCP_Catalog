...
export const NEPHILIM_SOURCE_NARRATIVES: Record<string, NephilimSourceNarrative> = {
  brave_mcp: {
    ...
    narrativeName: "Cipher's Archives",
    icon: '📚',
    color: '#2ecc71',
  ...
  mongodb_mcp: {
    ...
    narrativeName: "Aurora's Crystal Grid",
    icon: '🔮',
    color: '#f39c12',
...
// Get the narrative for a source type, with optional NEPHILIM mode
export const getSourceNarrative = (
  sourceType: string,
  isNephilimMode: boolean
): { label: string; description: string; icon: string; color: string } => {
  if (!isNephilimMode) {
    // Return standard labels for non-NEPHILIM mode
    const standardLabels: Record<string, { label: string; description: string; icon: string; color: string }> = {
      llm: { label: 'Pure LLM Response', description: 'Generated by AI', icon: '🧠', color: '#b07cc6' },
  ...
  const narrative = NEPHILIM_SOURCE_NARRATIVES[sourceType] || NEPHILIM_SOURCE_NARRATIVES.llm
  return {
    label: narrative.narrativeName,
...
// Format tool name for NEPHILIM display
export const formatToolNarrative = (toolName: string, isNephilimMode: boolean): string => {
  if (!isNephilimMode) {
    // Standard formatting
    return toolName.replace('bitcoin_', '').replace(/_/g, ' ')
  }

  const narrative = NEPHILIM_TOOL_NARRATIVES[toolName]
  return narrative?.narrativeName || toolName.replace('bitcoin_', '').replace(/_/g, ' ')
}
